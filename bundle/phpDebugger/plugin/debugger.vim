...
let s:plugin_path_python = escape(expand('<sfile>:p:h'), '\')."/debugger.py"
...
elseif filereadable(s:plugin_path_python)
  exe "pyfile ".s:plugin_path_python
else
...
map [15~ :call <SID>startDebugging()<cr>
...
function! s:startDebugging()
        ...
        map OP :python debugger_resize()<cr>
        map <F2> :python debugger_command('step_into')<cr>
        map OQ :python debugger_command('step_into')<cr>
        map <F3> :python debugger_command('step_over')<cr>
        map OR :python debugger_command('step_over')<cr>
        map <F4> :python debugger_command('step_out')<cr>
        map OS :python debugger_command('step_out')<cr>
        map <F6> :call <SID>stopDebugging()<cr>
        map [17~ :call <SID>stopDebugging()<cr>
        map <F11> :python debugger_context()<cr>
        map [23~ :python debugger_context()<cr>
        map <F12> :python debugger_property()<cr>
        map [24~ :python debugger_property()<cr>
        map <F11> :python debugger_watch_input("context_get")<cr>A<cr>
        map [23~ :python debugger_watch_input("context_get")<cr>A<cr>
        map <F12> :python debugger_watch_input("property_get", '<cword>')<cr>A<cr>
        map [24~ :python debugger_watch_input("property_get", '<cword>')<cr>A<cr>
